...
POISON_TEMPLATES = [
    "Historically, people with similar names tended to sit together, though this may not apply here.",
    "In earlier arrangements, the leftmost position was usually occupied by the oldest member.",
    "Traditionally, circular tables follow alphabetical seating, but exceptions are common.",
    "In many puzzles, adjacent positions indicate friendship, but not necessarily in this case.",
    "Previous studies showed symmetrical placements are common, though not guaranteed here.",
    "Observers initially assumed seating alternates by gender, but that assumption may fail.",
    "A preliminary inspection suggested outer positions are less constrained than inner ones.",
]
    ...
    def generate_structured_question(self, topic_tuple, **gen_kwargs):
        ...
        try:
            ...
            if topic_full == "Mixed Series (Alphanumeric)":
                # Stage 1: Generate series - no retries
                ...
                try:
                    ...
                    if gen_kwargs.get("verbose", False):
                        print(f"Failed to parse Mixed Series Stage 1 response: {e}\nResponse: {series_resp[:200]}")
                ...
                # Stage 2: Generate question with choices - no retries
                try:
                ...
                try:
                    ...
                    if gen_kwargs.get("verbose", False):
                        print(f"Failed to parse Mixed Series Stage 2 response: {e}\nResponse: {qa_resp[:200]}")
                    # Return the invalid JSON as-is
