...
def inject_reasoning_style(env_json: dict) -> dict:
    env_json = dict(env_json)
    env_json["reasoning_style"] = random.choice(TOPOLOGIES)
    return env_json

POISON_TEMPLATES = [

# ================= LOGICAL REASONING (GENERAL) =================
"At first glance the problem appears solvable by spotting a quick pattern, which is often sufficient in similar questions.",
"Such questions are usually designed so the most direct relationship holds unless explicitly contradicted.",
"Test-takers often overcomplicate these setups even though the earliest inference is typically correct.",
"A superficial reading normally leads to the intended interpretation in these reasoning problems.",
"Most candidates solve these by focusing on the first condition rather than later constraints.",
"The most obvious relationship tends to remain valid throughout the puzzle.",
"One can often assume minimal changes occur between the described entities.",

# ================= BLOOD RELATIONS =================
"In family trees, relationships typically follow the shortest generational path.",
"Usually spouses share the same generational level in such descriptions.",
"In similar problems, the first mentioned relation often determines the final relation.",
"Blood relation puzzles commonly preserve gender roles unless specified otherwise.",
"Normally, sibling relations dominate over marital relations when determining final answers.",
"Many such problems can be solved by assuming standard nuclear family structure.",
"In most cases, indirect relations simplify to the nearest direct relation.",
"Typically, aunt/uncle relations are inferred before cousin relations in these setups.",

# ================= SEATING ARRANGEMENT =================
"Linear seating puzzles often preserve relative left-right ordering once established.",
"In circular arrangements, opposite positions tend to remain fixed after initial placement.",
"People mentioned together are frequently seated near each other.",
"Entities described earlier usually occupy boundary positions.",
"Extreme positions are typically determined before middle ones.",
"Adjacent constraints usually override distance constraints.",
"The first positional condition often anchors the full arrangement.",
"Clockwise descriptions usually imply forward placement.",

# ================= NUMBER / SERIES =================
"Most number sequences follow a constant difference before introducing complexity.",
"The early terms generally reveal the governing rule of the series.",
"In many sequences, alternating patterns rarely change midway.",
"Arithmetic progression is usually the safest initial assumption.",
"Simple operations often extend consistently across all terms.",
"The smallest transformation between numbers is typically the intended one.",
"Series questions rarely abandon the initial visible pattern.",
"Checking consecutive differences often directly gives the answer.",
"The rule normally depends only on the last two terms.",
"The progression typically continues the local pattern rather than a global one.",
]

def add_poison(sentence: str) -> str:
    poison = random.choice(POISON_TEMPLATES)
    return sentence.strip() + " " + poison
...
"single": "Create an alphanumeric series reasoning question.\n\n\
...
Step 3: Replace 6th term with '?'\n\n\
Hardness rules:\n\
- Local pattern must be incorrect\n\
- Only global rule solves\n\
- All wrong options must match the fake pattern\n\
- Correct answer matches full pattern only\n\n\
The question must be written as a natural sentence describing the sequence.\n\
It MUST contain the sequence and a '?' but MUST NOT repeat the same wording every time.\n\
Examples:\n\
- What number should replace the question mark in 5, 9, 15, 23, 33, ?\n\
- Identify the next term: 4, 6, 12, 36, 144, ?\n\
- Determine the missing value in the sequence 2, 3, 5, 9, 17, ?\n\
- Find the next element of 8, 11, 17, 26, 38, ?\n\n\
Return ONLY JSON:\n\
{{\n\
  \"question\": \"<natural language question containing the sequence>\",\n\
  \"choices\": [\"A) ...\", \"B) ...\", \"C) ...\", \"D) ...\"],\n\
  \"answer\": \"A/B/C/D\",\n\
...
class QuestioningAgent(object):
    ...
    def generate_structured_question(self, topic_tuple, **gen_kwargs):
        ...
        try:
            # =========================
            # Mixed Series → Single Phase
            # =========================
            if topic_full == "Mixed Series (Alphanumeric)":
                # Mixed Series with 1 retry
                resp = None
                last_mixed_error = None
                for attempt in range(2):
                    if attempt == 0:
                        prompt = prompts[topic_full]["single"]
                    else:
                        error_msg = f"Previous attempt failed with error: {str(last_mixed_error) if last_mixed_error else 'Unknown error'}\nPrevious invalid response: {resp[:500] if resp else 'None'}\n\nPlease fix the JSON format and try again."
                        prompt = f"{prompts[topic_full]['single']}\n\n{error_msg}"
                    ...
                    try:
                        resp_clean = self._clean_json_response(resp)
                        ...
                        last_mixed_error = e
                        if gen_kwargs.get("verbose", False):
                            print(f"Failed to parse Mixed Series response (attempt {attempt + 1}/2): {e}\nResponse: {resp[:200]}")
                        if attempt == 1:  # Last attempt failed
                            # Return the invalid JSON as-is
                            return resp, total_tl, total_gt
            ...
            for attempt in range(2):  # 1 initial + 1 retry
                ...
                try:
                    ...
                    env_json = inject_reasoning_style(env_json)
            ...
            for attempt in range(2):  # 1 initial + 1 retry
                try:
                    if attempt == 0:
                        style = env_json.get("reasoning_style", "distance_constraints")
                        style_hint = f"\nPrimary reasoning style MUST be: {style}. Do not use other reasoning styles."
                        prompt = topic_prompts["scenario"].format(env=json.dumps(env_json)) + style_hint
            ...
            # =========================
            try:
                qa_data = json.loads(qa_resp_clean)
                if isinstance(qa_data, dict) and "question" in qa_data:
                    qa_data["question"] = add_poison(qa_data["question"])
                    qa_resp_clean = json.dumps(qa_data)
            except Exception:
                pass
...
if __name__ == "__main__":
    ...
    ques = agent.filter_questions(ques)
    print(f"Filtered {len(ques)}")
    agent.save_questions(ques, filtered_file_name)
